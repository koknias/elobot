...
def _pair_player_ids(matches: list[dict]) -> set[int]:
    """Extract all player ids from a list of matches (pair legs)."""
    ids: set[int] = set()
    for m in matches:
...
def _canonical_order_pairs(
    pairs: dict[tuple[int, int], list[dict]],
    prev_player_slot: dict[int, int] | None = None,
) -> list[tuple[tuple[int, int], list[dict]]]:
    """Return *pairs* sorted by canonical bracket slot order.

    For the first (base) stage, the DB insertion order *is* the bracket
    order because ``generate_playoff`` creates matches via
    ``_build_bracket_pairs`` which follows ``_bracket_seed_order``.

    For later stages, we use lineage: each pair in the current stage
    receives players who advanced from the previous stage.  The canonical
    slot ``k`` of a current-stage pair is ``prev_slot // 2`` where
    ``prev_slot`` is the bracket slot of either feeder pair in the
    previous stage.
    """
    if not prev_player_slot:
        # Base stage — DB insertion order = bracket order.
        return list(pairs.items())

    slot_items: list[tuple[int, tuple[int, int], list[dict]]] = []
    unplaced: list[tuple[tuple[int, int], list[dict]]] = []
    for key, ms in pairs.items():
        cand = [
        ...
        if cand:
            slot_items.append((min(cand), key, ms))
        else:
            unplaced.append((key, ms))

    slot_items.sort(key=lambda x: x[0])

    result: list[tuple[tuple[int, int], list[dict]]] = [
        (key, ms) for _, key, ms in slot_items
    ]
    # Fallback: any pair we couldn't position goes at the end.
    result.extend(unplaced)
    return result


def _canonical_slot_of_pair(
    ms: list[dict],
    prev_player_slot: dict[int, int],
) -> int | None:
    """Return the *real* bracket slot of a pair in the current stage.

    This is ``prev_slot // 2`` for either feeder in the previous stage.
    Returns ``None`` if the pair's players aren't in ``prev_player_slot``
    (which happens for the base stage or orphaned matches).
    """
    cand = [
        prev_player_slot[pid] // 2
        for pid in _pair_player_ids(ms)
...
def advance_playoff(tid: int) -> str | None:
    ...
    # Track the canonical bracket-slot of every player in the most
    # recently processed stage so that the *next* stage's pairs can be
    # sorted into canonical bracket order (not DB insertion order).
    prev_player_slot: dict[int, int] | None = None
    ...
    for s in PLAYOFF_STAGES:
        ...
        #
        # Sort pairs into canonical bracket order so that winners[i]
        # and winners[i+1] are truly adjacent bracket slots (not just
        # adjacent in DB insertion order, which can be scrambled when
        # pairs resolve at different times).
        ordered_pairs = _canonical_order_pairs(pairs, prev_player_slot)
        winners: list[int | None] = []
        pair_outcomes: list[tuple[int, int | None] | None] = []
        # Track the *real* bracket slot of each pair (with gaps for
        # not-yet-created pairs) so that next-stage matchmaking pairs
        # adjacent bracket slots, not adjacent list indices.
        slot_winner: dict[int, int | None] = {}
        slot_pairs_map: dict[int, tuple[tuple[int, int], list[dict]]] = {}
        for key, ms in ordered_pairs:
            ms_sorted = sorted(ms, key=lambda x: x.get("leg") or 1)
            a_id = ms_sorted[0]["player1_id"]
            b_id = ms_sorted[0]["player2_id"]
            real_slot = (
                _canonical_slot_of_pair(ms, prev_player_slot)
                if prev_player_slot else None
            )
            if a_id == b_id:
                # Bye row — winner = byed seed, no real loser.
                winners.append(a_id)
                pair_outcomes.append((a_id, None))
                if real_slot is not None:
                    slot_winner[real_slot] = a_id
                    slot_pairs_map[real_slot] = (key, ms)
                continue
            w = _resolve_pair_winner(
                ms_sorted, advance_mode=adv_mode, series_len=legs_target,
            )
            ...
            if real_slot is not None:
                slot_winner[real_slot] = w
                slot_pairs_map[real_slot] = (key, ms)

        all_done = all(w is not None for w in winners)

        # Build the player → slot mapping for this stage so the next
        # stage can sort its pairs into canonical bracket order.
        # Use the *real* bracket slot (with gaps for missing pairs) so
        # that next-stage matchmaking pairs adjacent bracket halves.
        cur_player_slot: dict[int, int] = {}
        if slot_pairs_map:
            for real_slot, (key, ms) in slot_pairs_map.items():
                for pid in _pair_player_ids(ms):
                    cur_player_slot[pid] = real_slot
        else:
            # Fallback: base stage or no prev_player_slot — use dense.
            for slot, (_key, ms) in enumerate(ordered_pairs):
                for pid in _pair_player_ids(ms):
                    cur_player_slot[pid] = slot
        # Propagate to next iteration.
        prev_player_slot = cur_player_slot
        ...
        # Create next-stage matches incrementally: for each adjacent
        # bracket-slot pair (2k, 2k+1) where BOTH winners are known,
        # create the match if it does not already exist.
        # Using real slots (with gaps) prevents cross-half pairing when
        # some pairs haven't been created yet (pending R32 matches).
        if slot_winner and prev_player_slot:
            # Use real bracket slots — pairs 2k↔2k+1 regardless of gaps.
            max_slot = max(slot_winner)
            for k in range(0, max_slot + 1, 2):
                if k not in slot_winner or k + 1 not in slot_winner:
                    continue
                a = slot_winner[k]
                b = slot_winner[k + 1]
            ...
            # Fallback (base stage): use dense winners list.
