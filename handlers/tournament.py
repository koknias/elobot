...
from rich_messages import RichMessageError, build_tournament_announcement_html, send_rich_html_message
...
async def cmd_start_tournament(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if int(t.get("bracket_only") or 0):
        ...
        await send(update, "\n".join(lines))
        await _try_send_rich_event(
            update.effective_chat.id if update.effective_chat else None,
            _playoff_started_rich_html(t, matches_info),
        )
    ...
    if manual_groups:
        ...
        await _try_send_rich_event(
            update.effective_chat.id if update.effective_chat else None,
            _groups_started_rich_html(t, groups, len(mids)),
        )
...
def _standings_rich_html(t: dict) -> str:
    standings = get_group_standings(t["id"])
    blocks = [f"<h2>📊 Турнирная таблица: {html.escape(t['name'])}</h2>"]
    custom_name = (t.get("group_display_name") or "").strip()
    for group_name, players in sorted(standings.items()):
        title = custom_name if custom_name and len(standings) == 1 else f"Группа {group_name}"
        rows = []
        for pos, p in enumerate(players, 1):
            played = p["group_wins"] + p["group_draws"] + p["group_losses"]
            goals = f"{p['group_gf']}:{p['group_ga']}"
            rows.append(
                f"<tr><td>{pos}</td><td>{html.escape(str(p['username']))}</td>"
                f"<td>{played}</td><td>{p['group_wins']}</td><td>{p['group_draws']}</td>"
                f"<td>{p['group_losses']}</td><td>{goals}</td><td>{p['group_points']}</td></tr>"
            )
        blocks.append(
            f"<table bordered striped><caption>{html.escape(title)}</caption>"
            "<tr><th>#</th><th>Игрок</th><th>И</th><th>В</th><th>Н</th><th>П</th><th>Г</th><th>О</th></tr>"
            + "".join(rows)
            + "</table>"
        )
    return "\n".join(blocks)
...
def _playoff_rich_html(t: dict) -> str:
    text = format_playoff_bracket(t["id"])
    sections: list[str] = []
    current_title = "Плей-офф"
    current_lines: list[str] = []
    for raw in text.splitlines()[1:]:
        line = raw.strip()
        if not line:
            continue
        is_stage = line.startswith("<b>") and "</b>" in line and not line.startswith("<b>Плей")
        if is_stage:
            if current_lines:
                sections.append(
                    f"<details open><summary>{current_title}</summary><ul>"
                    + "".join(f"<li>{x}</li>" for x in current_lines)
                    + "</ul></details>"
                )
            current_title = line
            current_lines = []
    ...
    return f"<h2>🏆 Плей-офф: {html.escape(t['name'])}</h2>\n" + "\n".join(sections)


async def _try_send_rich_event(chat_id: int | str | None, rich_html: str) -> None:
    if chat_id is None:
        return
    try:
        await send_rich_html_message(chat_id, rich_html)
    except RichMessageError as exc:
        log.warning("rich event post failed: %s", exc)


def _groups_started_rich_html(t: dict, groups: dict, match_count: int) -> str:
    blocks = [
        f"<h2>🎲 Жеребьёвка завершена: {html.escape(t['name'])}</h2>",
        "<table bordered striped><caption>Старт турнира</caption>"
        "<tr><th>Параметр</th><th>Значение</th></tr>"
        f"<tr><td>Тип</td><td>{t_full_label(t)}</td></tr>"
        f"<tr><td>Групп</td><td>{len(groups)}</td></tr>"
        f"<tr><td>Создано матчей</td><td>{match_count}</td></tr>"
        "<tr><td>Срок матча</td><td>48 часов</td></tr>"
        "</table>",
    ]
    for g, pids_g in sorted(groups.items()):
        items = []
        for pid in pids_g:
            p = get_player_by_id(pid)
            if p:
                items.append(f"<li>{mention(p['username'])}</li>")
        blocks.append(
            f"<details open><summary>Группа {html.escape(str(g))}</summary><ul>"
            + "".join(items)
            + "</ul></details>"
        )
    blocks.append(f"<footer>Таблица: /table_text {t['id']}. Матчи: /matches.</footer>")
    return "\n".join(blocks)


def _playoff_started_rich_html(t: dict, matches_info: list[dict]) -> str:
    rows = []
    seen: set[tuple[str, str]] = set()
    for mi in matches_info:
        a = str(mi.get("player1") or "")
        b = str(mi.get("player2") or "")
        key = tuple(sorted((a, b)))
        if key in seen:
            continue
        seen.add(key)
        rows.append(f"<tr><td>{mention(a)}</td><td>{mention(b)}</td></tr>")
    return (
        f"<h2>🏆 Плей-офф запущен: {html.escape(t['name'])}</h2>"
        "<table bordered striped><caption>Пары первого раунда</caption>"
        "<tr><th>Игрок 1</th><th>Игрок 2</th></tr>"
        + "".join(rows)
        + f"</table><footer>Сетка: /playoff_text {t['id']}.</footer>"
    )


def _finish_rich_html(t: dict) -> str:
    podium = get_tournament_podium(int(t["id"]))
    def label(pid):
        return _player_tag(pid, tid=int(t["id"])) if pid else "—"
    bronze = label(podium.get("third"))
    if not podium.get("third") and podium.get("third_tied"):
        bronze = ", ".join(label(pid) for pid in podium["third_tied"])
    return f"""
<h2>🏁 Итоги турнира: {html.escape(t['name'])}</h2>
<table bordered striped>
<tr><th>Место</th><th>Игрок</th></tr>
<tr><td>🥇 Чемпион</td><td>{label(podium.get('first'))}</td></tr>
<tr><td>🥈 Серебро</td><td>{label(podium.get('second'))}</td></tr>
<tr><td>🥉 Бронза</td><td>{bronze}</td></tr>
</table>
<footer>Сводка турнира отправлена ниже. Таблица: /table_text {t['id']}. Сетка: /playoff_text {t['id']}.</footer>
""".strip()
...
async def cmd_table_text(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if t is None:
        ...
        return
    try:
        await send_rich_html_message(update.effective_chat.id, _standings_rich_html(t))
    except RichMessageError as exc:
        log.warning("standings rich message failed: %s", exc)
        await send(update, format_standings_message(t["id"]))
...
async def cmd_playoff_text(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if t is None:
        ...
        return
    try:
        await send_rich_html_message(update.effective_chat.id, _playoff_rich_html(t))
    except RichMessageError as exc:
        log.warning("playoff rich message failed: %s", exc)
        await send(update, format_playoff_bracket(t["id"]))
...
async def cmd_fill_missing_matches(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # Guard: don't create group-stage matches when tournament has moved to playoff
    t_stage = (t.get("stage") or "groups").lower()
    if t_stage in ("playoff", "finished", "groups_done"):
        await send(
            update,
            f"❌ Турнир <b>{html.escape(t['name'])}</b> (ID {tid}) "
            f"находится на этапе «{t_stage}». "
            f"Создание групповых матчей недоступно.",
        )
        return
