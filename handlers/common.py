...
from rich_messages import (
    RichMessageError,
    build_auto_rich_html,
    send_rich_html_message,
    should_auto_rich_text,
)
...
async def send(update: Update, text: str, **kwargs):
    ...
    if parse_mode == "HTML" and should_auto_rich_text(text):
        rich_kwargs = dict(kwargs)
        rich_kwargs.pop("parse_mode", None)
        link_preview_options = rich_kwargs.pop("link_preview_options", None)
        rich_kwargs.pop("disable_web_page_preview", None)
        rich_html = build_auto_rich_html(text)
        if rich_html:
            try:
                await send_rich_html_message(
                    target.chat_id,
                    rich_html,
                    reply_markup=rich_kwargs.get("reply_markup"),
                )
                return
            except RichMessageError as exc:
                log.warning("auto rich message failed, falling back: %s", exc)
            finally:
