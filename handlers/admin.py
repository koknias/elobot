...
async def cmd_announce_rich(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """``/announce_rich [ID]`` — post a Bot API 10 rich tournament card."""
    t, err = _resolve_tournament_from_args(update, ctx)
    if not t:
        await send(update, err or "❌ Не нашёл турнир.")
        return
    if not _can_manage_tournament(update.effective_user.id, t):
        await send(update, "❌ Анонс отправляет создатель или админ турнира.")
        return
    chat = update.effective_chat
    if not chat:
        await send(update, "❌ Не вижу чат для отправки анонса.")
        return

    rich_html = build_tournament_announcement_html(t, type_label=t_type_label)
    try:
        await send_rich_html_message(chat.id, rich_html)
    except RichMessageError as exc:
        await send(
            update,
            "⚠️ Telegram не принял rich-сообщение. Отправляю обычный вариант.\n"
            f"<code>{html.escape(str(exc)[:700])}</code>",
        )
        await send(update, build_plain_tournament_announcement(t, type_label=t_type_label))
        return
    await send(update, "✅ Rich-анонс турнира отправлен.")


async def _send_rich_or_plain_tournament_card(update, t: dict, *, kind: str):
    chat = update.effective_chat
    if not chat:
        await send(update, "❌ Не вижу чат для отправки сообщения.")
        return
    title = "Регламент" if kind == "rules" else "Информация о турнире"
    footer = "Используйте /matches, /table и /playoff для текущего состояния."
    if kind == "rules":
        footer = "Регламент можно обновить через /set_description."
    rich_html = build_tournament_announcement_html(
        t,
        type_label=t_type_label,
        footer=footer,
    ).replace("Регистрация открыта", title, 1)
    try:
        await send_rich_html_message(chat.id, rich_html)
    except RichMessageError as exc:
        log.warning("rich tournament card failed: %s", exc)
        await send(update, build_plain_tournament_announcement(t, type_label=t_type_label, footer=footer))


async def cmd_reglament(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """``/reglament [ID]`` — show the tournament regulation as a rich card."""
    t, err = _resolve_tournament_from_args(update, ctx)
    if not t:
        await send(update, err or "❌ Не нашёл турнир.")
        return
    await _send_rich_or_plain_tournament_card(update, t, kind="rules")


async def cmd_tournament_info(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """``/tournament_info [ID]`` — show tournament status as a rich card."""
    t, err = _resolve_tournament_from_args(update, ctx)
    if not t:
        await send(update, err or "❌ Не нашёл турнир.")
        return
    await _send_rich_or_plain_tournament_card(update, t, kind="info")
