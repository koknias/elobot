    ...
    try:
        await send_rich_html_message(
            ...
            _profile_rich_html(
                ...
                title_strs=uniq_t if title_strs else [],
            ),
            reply_markup=kb,
        )
        return
    except RichMessageError as exc:
        log.warning("profile rich message failed: %s", exc)
    try:
...
async def cmd_matches(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await send_rich_html_message(
            update.effective_chat.id,
            _matches_rich_html(p, history),
        )
    except RichMessageError as exc:
        log.warning("matches rich message failed: %s", exc)
        await send(update, "\n".join(lines))


async def cmd_howto(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """``/howto`` — rich onboarding checklist for new players."""
    rich_html = """
<h2>🚀 Как начать играть</h2>
<table bordered striped>
<caption>Быстрый старт</caption>
<tr><th>Шаг</th><th>Что сделать</th></tr>
<tr><td>1</td><td>/register — зарегистрироваться в боте</td></tr>
<tr><td>2</td><td>/setnick НикВИгре — привязать игровой ник</td></tr>
<tr><td>3</td><td>/tournaments — выбрать турнир и нажать «Записаться»</td></tr>
<tr><td>4</td><td>/matches — посмотреть свои матчи</td></tr>
<tr><td>5</td><td>/report 3:2 @opponent — отправить результат</td></tr>
</table>
<details open><summary>Важно</summary><ul>
<li>Проверь Telegram-привязку перед регистрацией.</li>
<li>Скриншоты матчей отправляй в привязанный чат турнира.</li>
<li>Если есть спор — используй /dispute.</li>
</ul></details>
""".strip()
    try:
        await send_rich_html_message(update.effective_chat.id, rich_html)
    except RichMessageError as exc:
        log.warning("howto rich message failed: %s", exc)
        await send(
            update,
            "🚀 <b>Как начать играть</b>\n\n"
            "1. /register — зарегистрироваться\n"
            "2. /setnick НикВИгре — привязать игровой ник\n"
            "3. /tournaments — выбрать турнир\n"
            "4. /matches — посмотреть матчи\n"
            "5. /report 3:2 @opponent — отправить результат",
        )
