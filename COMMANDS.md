...
Все команды бота. Часть команд отправляет Telegram Rich Messages: красивые карточки, таблицы и раскрываемые блоки. Если клиент Telegram или Bot API не принимает rich-формат, бот автоматически отправляет обычный HTML-текст.
...
| `/howto`, `/how_to`, `/start_guide` | Красивая инструкция для новичка: регистрация, ник, запись, матчи, репорт результата. |
...
| `/match_preview`, `/opponent_preview <@user>` | Rich-превью матча против соперника: ELO, форма, статистика. |
...
| `/reglament`, `/rules [ID]` | Rich-регламент турнира: формат, правила, настройки, раскрываемые блоки. |
| `/tournament_info`, `/tinfo [ID]` | Rich-карточка турнира: статус, формат, ссылки на таблицу/сетку. |
...
| `/top_week` | Rich-топ недели по подтверждённым матчам. |
| `/top_month` | Rich-топ месяца по подтверждённым матчам. |
...
| `/match`, `/match_card <ID>` | Rich-карточка конкретного матча: игроки, стадия, статус, счёт, дедлайн. |
...
| `/announce_rich`, `/rich_announce`, `/tournament_announce [ID]` | Опубликовать красивый rich-анонс турнира. |
| `/final_post`, `/tournament_final [ID]` | Rich-финальный пост турнира: чемпион, серебро, бронза. |
| `/tournament_admin`, `/tadmin_panel [ID]` | Rich-мини-панель админа турнира: стадия, регистрация, автоподтверждение, матчи. |
...
| `/quote_settings, /quotemenu, /цитаты` | 🎛 Кнопочное меню настроек цитат для этого чата (текущая частота, кол-во цитат, пресеты `Выкл / 30мин / 1ч / 3ч / 6ч / 12ч / 24ч`). |
| `/bug, /bugreport, /баг [текст]` | 🐞 Сообщить о баге. Без аргументов — режим ожидания (следующее сообщение/фото уйдёт админам). С тегом «BUG REPORT» в DM админам, отдельно от `/feedback`. |
...
| `/today` | Rich-таблица матчей с дедлайнами на ближайшие 24 часа. |
| `/overdue` | 👮 Rich-таблица просроченных матчей. |
| `/violation <@user> <причина>` | 👮 Rich-карточка нарушения регламента. |
