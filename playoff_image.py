    ...
    *,
    ...
    for stage, pairs in other_stages:
        if len(pairs) <= 1:
            ...
            # With an odd number of pairs (e.g. 3 QF pairs where one
            # is a TBD placeholder), split so the top half gets the
            # extra pair: ceil(n/2) top, floor(n/2) bottom. Otherwise
            # the bottom half would have 2 pairs and the top only 1,
            # making a real match appear in the wrong vertical position.
            mid = (len(pairs) + 1) // 2
