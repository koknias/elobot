...
# ── AI Vision OCR (Groq + Gemini + NVIDIA NIM + OpenRouter + OCR.space) ─────
...
#   3. NVIDIA NIM   — hosted models via build.nvidia.com (5000 req/month free).
#   4. OpenRouter   — multi-model free chain with key rotation on 429.
#   5. OCR.space    — last-resort plain OCR (score-only, no team/league/goals).
...
# Primary key: env OPENROUTER_API_KEY.
...
_OPENROUTER_DEFAULT_KEY = ""
_OPENROUTER_API_KEY_2 = os.getenv("OPENROUTER_API_KEY_2", "").strip()
_OPENROUTER_API_KEY_3 = os.getenv("OPENROUTER_API_KEY_3", "").strip()
...
# NVIDIA NIM (build.nvidia.com) — free endpoint for hosted models.
# Free-tier limits: 5000 API calls/month, no card required.
# https://build.nvidia.com/docs/api
_NVIDIA_NIM_API_KEY = os.getenv(
    "NVIDIA_NIM_API_KEY",
    "",
).strip()
_NVIDIA_NIM_URL = "https://integrate.api.nvidia.com/v1/chat/completions"
# Free multimodal models available via NVIDIA NIM (best for OCR):
#   - nvidia/nemotron-nano-12b-v2-vl — same model as the primary, but
#     served directly from NVIDIA infra (no OpenRouter middleman). ~1s.
#   - google/paligemma — Google's multimodal model (confirmed multimodal).
#   - meta/llama-3.2-11b-vision-instruct — lightweight fast vision model.
#   - microsoft/phi-4-multimodal-instruct — multimodal reasoning model.
_NVIDIA_NIM_MODELS = (
...
Schema:
{
  ...
  "timer":  str,                    // match clock shown on screen: "90:00", "120:00", etc.
...
PENALTIES (important — penalty shootouts are RARE and ONLY happen
when the match timer shows "120:00" after extra time):
...
CRITICAL — 120:00 RULE: Penalty shootout numbers appear ONLY when the
match timer shows "120:00". Check the timer on the screen:
  • Timer = "120:00" + draw score → penalties ARE present, output pen1/pen2
  • Timer = "90:00" (or any non-120 time) + draw score → NO penalties,
    set pen1=null, pen2=null. A 2:2 draw at 90:00 is just a regular draw.

DO NOT hallucinate penalty numbers from other UI elements (statistics,
badge numbers, goal counts). The parenthesised penalty numbers are ONLY
present when the timer shows 120:00.

Rules for pen1 / pen2:
- FIRST check the match timer. If it does NOT show "120:00", set
  pen1=null and pen2=null — there are no penalties.
- If the timer shows "120:00" AND the score is a draw, look for the two
  small numbers in parentheses flanking the big score digits.
- Output integers ONLY when BOTH parenthesised numbers are clearly
  visible, sit on the same row as the big score, and the regulation
  score is a draw (score1 == score2). Otherwise BOTH must be null.
...
- Match-clock readings ("90:00", "120:00") are NOT penalty scores —
  ignore the timer value itself.
- If you cannot tell whether the small numbers in parens are penalty
  results, set pen1 and pen2 to null. Do not guess.
- For a normal match without a shootout (timer ≠ 120:00), set
  pen1=null and pen2=null.

WARNING — DO NOT confuse stats with penalties:
The STATS panel (below the score) shows match statistics like:
  "УДАРЫ (В СТВОР): 5(4) — 0(0)"
  "ВЛАДЕНИЕ: 44% — 56%"
These numbers in parentheses are SHOTS ON TARGET, NOT penalty shootout
scores! NEVER use stats numbers for pen1/pen2. Penalty numbers are ONLY
the small digits in parentheses flanking the BIG central score, and ONLY
when the timer shows 120:00. If you see "(4)" in the stats section, that
is shots on target — ignore it for penalties.
...
def _ai_parse_response(content: str) -> dict:
    ...
    return {
        ...
        "timer":  (str(obj.get("timer") or "").strip() or None),
...
def _ai_post_process(parsed: dict) -> None:
    ...
    # (e) the match timer shows "120:00" — penalties ONLY happen after
    # extra time. If the timer shows 90:00 or anything else, drop pens.
    s1 = parsed.get("score1")
    s2 = parsed.get("score2")
    p1 = parsed.get("pen1")
    p2 = parsed.get("pen2")
    timer = str(parsed.get("timer") or "").strip()
    drop_pens = False
    if p1 is None or p2 is None:
        ...
        drop_pens = True
    elif not timer or "120" not in timer:
        # Penalties only happen after extra time (120:00). If the model
        # didn't report a timer, or the timer doesn't show 120:00,
        # drop penalty values — they're almost certainly hallucinated
        # from stats like "УДАРЫ (В СТВОР): 5(4) — 0(0)".
        drop_pens = True
    if drop_pens:
        parsed["pen1"] = None
        parsed["pen2"] = None
...
def _nvidia_nim_call_one(image_b64: str, model: str | None = None,
                         timeout: float = 30.0):
    """Run a single NVIDIA NIM vision call. Returns (parsed, raw, elapsed).

    NVIDIA NIM (build.nvidia.com) exposes hosted models via an
    OpenAI-compatible API.  Free tier: 5000 req/month, no card.
    https://build.nvidia.com/docs/api
    """
    chosen = (model or _NVIDIA_NIM_MODELS[0]).strip()
    body = json.dumps({
        "model": chosen,
        "messages": [{
            "role": "user",
    ...
    req = urllib.request.Request(
        ...
        data=body,
        headers={
            "Authorization": f"Bearer {_NVIDIA_NIM_API_KEY}",
            "Content-Type": "application/json",
        },
    )
    t0 = time.time()
    ...
    try:
        ...
        raise ValueError(f"NVIDIA NIM non-JSON response: {body_b[:200]!r}") from e
    if not isinstance(data, dict):
        raise ValueError(f"NVIDIA NIM non-object response: {str(data)[:200]!r}")
    if data.get("error"):
        raise ValueError(f"NVIDIA NIM API error: {str(data.get('error'))[:200]!r}")
    choices = data.get("choices")
    if not isinstance(choices, list) or not choices:
        raise ValueError(f"NVIDIA NIM returned no choices: {str(data)[:200]!r}")
    first = choices[0]
    if not isinstance(first, dict):
        raise ValueError(f"NVIDIA NIM choice[0] not a dict: {str(first)[:200]!r}")
    message = first.get("message") or {}
    msg = message.get("content") if isinstance(message, dict) else ""
    if isinstance(msg, list):
        msg = " ".join(
...
def ai_read_screenshot(image_bytes: bytes, model: str | None = None,
    ...
    Provider order (when ``models_override`` is NOT given):
      ...
      3. NVIDIA NIM — hosted models (5000 req/month free).
      4. OpenRouter free-models chain with key rotation on 429.
      5. OCR.space (last resort — score only, no team/league/goals).
    ...
    # ── 3. NVIDIA NIM (build.nvidia.com, 5000 req/month free) ──
    if _NVIDIA_NIM_API_KEY and not models_override:
        for nmodel in _NVIDIA_NIM_MODELS:
            for attempt in range(2):
                try:
                    parsed, raw, dt = _nvidia_nim_call_one(
                        image_b64, model=nmodel, timeout=timeout,
                    )
                    parsed["model"] = f"nvidia_nim/{nmodel}"
                    parsed["raw"] = raw
                    parsed["elapsed_s"] = dt
                    _ai_post_process(parsed)
                    log.info("NVIDIA NIM OCR ok via %s in %.1fs", nmodel, dt)
                    return parsed
                except urllib.error.HTTPError as e:
                    try:
                        err_body = e.read()[:200]
                        ...
                        err_body = b""
                    log.warning("nvidia_nim %s -> HTTP %s: %.200s",
                                nmodel, e.code, err_body)
                    if attempt == 0 and e.code == 429:
                        time.sleep(2)
                        continue
                    break  # non-retryable, try next model
                except Exception as e:
                    log.warning("nvidia_nim %s -> %r", nmodel, e)
                    break

    # ── 4. OpenRouter fallback chain ──
    seen: set[str] = set()
    candidates: list[str] = []
    if models_override:
    ...
    # ── 5. OCR.space last-resort score-only fallback ──
        ...
        try:
            if ai_is_available():
                ...
                if (
                    ...
                    # BUT: only run when the AI confirmed the timer shows
                    # 120:00 (extra time). Without this gate, Tesseract
                    # picks up stats numbers like "УДАРЫ (В СТВОР): 5(4)"
                    # and hallucinates penalty scores.
                    _ai_timer = ai.get("timer") or ""
                    if (
