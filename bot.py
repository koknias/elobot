...
from rich_messages import (  # noqa: E402
    RichMessageError,
    build_plain_tournament_announcement,
    build_tournament_announcement_html,
...
# ─────────────────────────────────────────────────────────────────────────────
# Background job: daily DB backup to root admins
# ─────────────────────────────────────────────────────────────────────────────

async def job_daily_backup(ctx: ContextTypes.DEFAULT_TYPE):
    """Every 24h, export the full DB and send the ZIP to each root admin (ADMIN_IDS) via DM."""
    if not ADMIN_IDS:
        return
    try:
        ...
        log.exception("job_daily_backup: export_database failed")
        return
    ...
    for admin_id in ADMIN_IDS:
        try:
            ...
            log.exception("job_daily_backup: failed to send to admin %s", admin_id)
        # reset stream for next recipient
        bio.seek(0)
    ...
    for t in active:
        ...
        if interval_min > 0 and t.get("chat_id"):
            ...
            if signups_open and _reminder_should_fire(
                ...
                if deadline_raw:
                    try:
                        deadline_label = f"{_fmt_minute_local(deadline_raw)} {_tz_label()}"
                    except Exception:
                        deadline_label = str(deadline_raw)
                ...
                try:
                    ...
                    _signup_footer = None
                rich_html = build_tournament_announcement_html(
                    t,
                    type_label=t_type_label,
                    registered_count=len(members),
                    deadline=deadline_label,
                    signup_link=link_raw or None,
                    footer=_signup_footer,
                )
                try:
                    ...
                    log.warning(
                        "signup rich reminder failed for tournament %s: %s",
                        tid,
                        exc,
                    )
                    try:
                    ...
                    log.exception("signup rich reminder failed for tournament %s", tid)
...
def main():
    ...
    app.add_handler(CommandHandler("how_to", cmd_howto))
    app.add_handler(CommandHandler("start_guide", cmd_howto))
    app.add_handler(CommandHandler("match", cmd_match_card))
    app.add_handler(CommandHandler("match_card", cmd_match_card))
    app.add_handler(CommandHandler("today", cmd_today))
    app.add_handler(CommandHandler("overdue", cmd_overdue_rich))
    app.add_handler(CommandHandler("tournament_admin", cmd_tournament_admin_rich))
    app.add_handler(CommandHandler("tadmin_panel", cmd_tournament_admin_rich))
    app.add_handler(CommandHandler("violation", cmd_violation))
    app.add_handler(CommandHandler("top_week", cmd_top_week))
    app.add_handler(CommandHandler("top_month", cmd_top_month))
    app.add_handler(CommandHandler("match_preview", cmd_match_preview))
    app.add_handler(CommandHandler("opponent_preview", cmd_match_preview))
    app.add_handler(CommandHandler("final_post", cmd_final_post_rich))
    app.add_handler(CommandHandler("tournament_final", cmd_final_post_rich))
    ...
    app.add_handler(CommandHandler("announce_rich", cmd_announce_rich))
    app.add_handler(CommandHandler("rich_announce", cmd_announce_rich))
    app.add_handler(CommandHandler("tournament_announce", cmd_announce_rich))
    app.add_handler(CommandHandler("reglament", cmd_reglament))
    app.add_handler(CommandHandler("rules", cmd_reglament))
    app.add_handler(CommandHandler("tournament_info", cmd_tournament_info))
    app.add_handler(CommandHandler("tinfo", cmd_tournament_info))
    ...
    # Daily DB backup — export + DM to every root admin (ADMIN_IDS)
    app.job_queue.run_repeating(job_daily_backup, interval=24 * 3600, first=60)
